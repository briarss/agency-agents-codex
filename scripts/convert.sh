...
#   codex        — Codex native agents + skills (agents/*.toml, skills/*/SKILL.md)
...
toml_escape_basic() {
  printf '%s' "$1" | perl -CS -pe 's/\\/\\\\/g; s/"/\\"/g; s/([\x00-\x1F])/sprintf("\\u%04X", ord($1))/eg'
}

toml_escape_multiline_basic() {
  perl -CS -0pe 's/\\/\\\\/g; s/"/\\"/g; s/([\x00-\x08\x0B\x0C\x0D\x0E-\x1F])/sprintf("\\u%04X", ord($1))/eg'
}
...
convert_codex() {
  local file="$1"
  local name description slug outfile body relpath instructions escaped_instructions skilldir skill_description
...
You are the ${name} specialist from The Agency.
...
HEREDOC
  ...
  escaped_instructions="$(printf '%s' "$instructions" | toml_escape_multiline_basic)"

  # Codex native subagent format: TOML files in ~/.codex/agents/.
  # The agency- prefix avoids collisions with existing Codex/OMX roles such as code-reviewer.
  cat > "$outfile" <<HEREDOC
# Generated by scripts/convert.sh. Do not edit manually.
# Source: ${relpath}
name = "${slug}"
description = "$(toml_escape_basic "$description")"
model = "gpt-5.4-mini"
...
HEREDOC
  ...
  skilldir="$OUT_DIR/codex/skills/$slug"
  mkdir -p "$skilldir"
  skill_description="$(yaml_escape_single_quoted "Agency specialist wrapper for ${name}. Delegates to native subagent ${slug} when available; otherwise runs the same role inline.")"

  cat > "$skilldir/SKILL.md" <<HEREDOC
---
name: ${slug}
...
Generated by scripts/convert.sh from source file: ${relpath}.

## Use When

Use this skill when the user explicitly asks for ${name}, \$${slug}, or needs this specialist's domain.

## Execution

1. Treat this skill as a specialist role activation, not a top-level workflow.
2. If native subagent delegation is available, spawn agent type ${slug} with the user's concrete task.
...
HEREDOC
  ...
  printf -- '- %s - %s: %s\n' "$slug" "$name" "$description" >> "$CODEX_ROSTER_TMP"
}
...
write_codex_router() {
  local router_dir="$OUT_DIR/codex/skills/agency"
  mkdir -p "$router_dir"

  cat > "$router_dir/SKILL.md" <<HEREDOC
---
name: agency
...
Generated by scripts/convert.sh.

## Use When

Use \$agency when the user wants one of The Agency specialists, asks which specialist fits a task, or references an Agency role without remembering the exact slug.
...
main() {
  ...
  if $use_parallel && [[ "$tool" == "all" ]]; then
    ...
    for t in "${tools_to_run[@]}"; do
      ...
      if [[ "$t" == "codex" ]]; then
        write_codex_router
        info "Wrote Codex native agents and skill wrappers"
      fi
